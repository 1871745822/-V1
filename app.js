...
const state = {
  ...
  titleState: null,
  // 白名单员工：在 Sheet2 中的员工不触发异常预警
  whitelistEmployees: new Set(),
};
...
function showShareQrModal(url, snapshotEncoded) {
  ...
  // 填充方式二的快照字符串
  const snapshotInput = $("shareSnapshotInput");
  if (snapshotInput && snapshotEncoded) {
    snapshotInput.value = snapshotEncoded;
  }

  // 清空固定链接输入框（等待用户填写 GitHub Pages URL）
  const fixedUrlInput = $("shareFixedUrlInput");
  if (fixedUrlInput) fixedUrlInput.value = "";
...
function computeAnomalies(rows, m) {
  ...
  const alerts = [];
  const whitelist = state.whitelistEmployees || new Set();
  for (const [key, v] of empMap.entries()) {
    // 白名单检查：员工在白名单中则跳过异常检测
    if (whitelist.has(v.emp)) continue;
...
async function parseFiles(fileList) {
  ...
  // 重置白名单
  state.whitelistEmployees = new Set();
  ...
  for (const f of files) {
    ...
    // === 读取 Sheet2（或第二个 sheet）提取白名单员工 ===
    // 优先查找名为 "Sheet2" 的 sheet，否则使用第二个 sheet
    const sheet2Name = sheets.find((s) => s.toLowerCase() === "sheet2") || sheets[1];
    if (sheet2Name && sheet2Name !== best.sheetName) {
      const wsSheet2 = wb.Sheets[sheet2Name];
      if (wsSheet2) {
        try {
          const parsedSheet2 = parseSheetToJson(wsSheet2, { maxDataRows: null });
          // 尝试找到员工姓名列
          const sheet2Mapping = guessMapping(parsedSheet2.cols);
          const empCol = sheet2Mapping.empCol;
          if (empCol && parsedSheet2.rows) {
            for (const row of parsedSheet2.rows) {
              const empName = (row?.[empCol] ?? "").toString().trim();
              if (empName) {
                state.whitelistEmployees.add(empName);
              }
            }
          }
        } catch {
...
async function init() {
  ...
  if (view) {
    ...
    if (!enc) {
      ...
      if (snapUrl) {
        try {
          const el = $("mappingHint");
          if (el) el.textContent = "正在从远程加载快照数据...";
          ...
          enc = text?.trim();
          if (!enc) {
            throw new Error("快照文件为空");
          }
        } catch (err) {
          const el = $("mappingHint");
          if (el) el.textContent = `加载快照失败：${err?.message || "网络错误"}。请检查链接是否正确。`;
          return;
        }
      }
    }
    ...
    // 恢复标题状态：优先使用快照中的标题，否则使用默认标题（不使用 localStorage）
    if (snap.titleState && Object.keys(snap.titleState).length > 0) {
      state.titleState = { ...defaultTitles, ...snap.titleState };
    } else {
      state.titleState = { ...defaultTitles };
    }
    // 恢复白名单员工
    state.whitelistEmployees = new Set(snap.whitelistEmployees || []);
      ...
      shareBtn.addEventListener("click", async () => {
        ...
        const buildSnapForKeys = async (periodKeys) => {
          ...
          return {
            ...
            p,
            // 白名单员工：存储为数组
            whitelistEmployees: Array.from(state.whitelistEmployees || []),
            // 标题状态：保存用户自定义的标题
        ...
        let shareUrl = "";
        let snapshotEncoded = ""; // 保存快照字符串供方式二使用
        ...
        for (const periodKeys of tryKeySets) {
          ...
          snapshotEncoded = await encodeShareData(snap);
          const u = `${viewBase}#${SHARE_PARAM_KEY}=${snapshotEncoded}`;
        ...
        // 展示二维码，便于对方扫码查看
        // 同时填充方式二的快照字符串输入框
        showShareQrModal(shareUrl, snapshotEncoded);
      });
    }
  ...
  // 方式二：复制快照字符串
  const copySnapshotBtn = $("btnCopySnapshot");
  if (copySnapshotBtn) {
    copySnapshotBtn.addEventListener("click", async () => {
      const input = $("shareSnapshotInput");
      const text = input?.value || "";
      if (!text) {
        alert("请先点击分享按钮生成快照数据。");
        return;
      }
      try {
        await navigator.clipboard.writeText(text);
        alert("快照字符串已复制。请将其保存到 GitHub 仓库的 latestSnapshot.txt 文件中。");
      } catch {
        window.prompt("复制快照字符串：", text);
      }
    });
  }

  // 方式二：复制固定链接（已预设好，可直接复制）
  const copyFixedUrlBtn = $("btnCopyFixedUrl");
  if (copyFixedUrlBtn) {
    copyFixedUrlBtn.addEventListener("click", async () => {
      const input = $("shareFixedUrlInput");
      const url = input?.value || "";
      if (!url) {
        alert("链接内容为空。");
        return;
      ...
      try {
        await navigator.clipboard.writeText(url);
        alert("固定链接已复制！将此链接发送给他人即可查看你的看板数据。");
      } catch {
        window.prompt("复制固定链接：", url);
      }
    });
  }
